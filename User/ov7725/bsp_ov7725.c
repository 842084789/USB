...
{
	uint8_t Start_flag [2] = {0x01,0x01};
	uint16_t i, j; 
	//ILI9341_OpenWindow(sx,sy,width,height);
	//ILI9341_Write_Cmd ( CMD_SetPixel );	
	
	//CDC_Send_DATA ((unsigned char*)Start_flag,2);
		...
		for(i = 0; i < width; i++)
		{
			...
			//USB_sendData (Camera_Data, height,8,j);
			//p[i][j]=Camera_Data;
			//p[i][j]=0xff00;
			
			//Usart_SendHalfWord(DEBUG_USARTx, Camera_Data);
			//Usart_SendHalfWord(DEBUG_USARTx, w);
			//SEND_FIFO_PIXEL(Camera_Buffer);
			SEND_FIFO_PIXEL(Camera_Buffer, j);
			while(Camera_send_flag == 1);
			}
...
void SEND_FIFO_PIXEL(uint8_t *buffer, uint8_t pixel_cnt)
{
	FIFO_RCLK_L();
	buffer[pixel_cnt*2] = ((OV7725_DATA_GPIO_PORT->IDR) & 0xff00)>>8;
	//Usart1SendByte(DEBUG_USARTx,buffer[1]);
	FIFO_RCLK_H();
  FIFO_RCLK_L();
	buffer[pixel_cnt*2+1] = (OV7725_DATA_GPIO_PORT->IDR >>8) & 0xff;
	//Usart1SendByte(DEBUG_USARTx,buffer[0]);
	FIFO_RCLK_H();
